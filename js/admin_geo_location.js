...
**********************************/
(function($) {
	$( 'div.geolocation' ).livequery( function() {
		$( this ).each( function() {
			var $this = $( this );

			google.load( 'maps', 3, { other_params: 'sensor=false', callback: function() {
				var latField = $this.find( 'input.geo-location-latitude-field' );
				var lngField = $this.find( 'input.geo-location-longitude-field' );
				var autoLocField = $this.find( 'input.geo-location-automatic-location-field' );

				var defaults = {
					$this.data( 'data-default-lat' );
					$this.data( 'data-default-lng' );
					$this.data( 'data-default-zoom' );
					
				};
			} } );
		} );
	} );
	// var geoLocationFieldCallback = function() {
	// 	$( 'div.geolocation' ).each( function(){
	// 		var $this = $( this );
	// 	});
	// };
	// google.load( 'maps', 3, { other_params: 'sensor=false', callback: geoLocationFieldCallback});
})(jQuery);
